...
*/
//Start speed encoder
int encoder_pinl = 2;  // The pin the encoder is connected       
int encoder_pinr = 3;  // The pin the encoder is connected           
unsigned int rpml;     // rpm reading
unsigned int rpmr;     // rpm reading
volatile byte pulsesl;  // number of pulses
volatile byte pulsesr;  // number of pulses
...
#define TRIG_PIN A0 // Pin A4 on the Motor Drive Shield soldered to the ultrasonic sensor
#define ECHO_PIN A1 // Pin A5 on the Motor Drive Shield soldered to the ultrasonic sensor
#define MAX_DISTANCE 200 // sets maximum useable sensor measuring distance to 200cm
#define MAX_SPEED 100 // sets speed of DC traction motors to 180/256 or about 70% of full speed - to get power drain down.
...
//------------------------------------------ SPEED ENCODER-------------------------------------------------------------------------------
 Serial.begin(9600);
   ...
   attachInterrupt(0, counter_L, FALLING);
   attachInterrupt(1, counter_R, FALLING);
   // Initialize
   pulsesl = 0;
   pulsesr = 0;
   rpml = 0;
...
if (millis() - timeold >= 1000){  /*Uptade every one second, this will be equal to reading frecuency (Hz).*/
  ...
  rpml = (60 * 1000 / pulsesperturn )/ (millis() - timeold)* pulsesl;
  rpmr = (60 * 1000 / pulsesperturn )/ (millis() - timeold)* pulsesr;
   timeold = millis();
   pulsesl = 0;
   pulsesr =0;
