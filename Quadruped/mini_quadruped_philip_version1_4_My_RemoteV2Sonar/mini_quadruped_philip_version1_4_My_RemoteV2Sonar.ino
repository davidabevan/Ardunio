...
///    zer0tilt v1.4 13-Oct-2014                   ///
///     - Recoded a working laydown()              ///
///     - Added InfraRed control                   ///
///   Status:
//flatout(); //done
//stand(); //done
//standup(); //done
...
/*
Since building it, I was subsequently forced to re-engineer
the code for forward walking, reverse, left turn and right turn.
...
int RECV_PIN = 2;
String command;
String SaveForKeepPressingCommand;
IRrecv irrecv(RECV_PIN);
decode_results results;
/*
// Define a few commands from your remote control
#define IR_CODE_FORWARD "9986485a" //Up
#define IR_CODE_BACKWARDS "1c6a919c" //Down
#define IR_CODE_TURN_LEFT "4eba0422" //Left
#define IR_CODE_TURN_RIGHT "5ea08104" //Right
#define IR_CODE_FLAT_OUT "4d480cab" //Mute
#define IR_CODE_STAND_UP "c20daa4f" //Vol+
#define IR_CODE_LAY_DOWN "4d6e48c9" //Vol-
#define IR_CODE_WINK "688ca8ee" //Optical
#define IR_CODE_STRETCH "6af21320" //Enter
#define IR_CODE_STAND "3731a63e" //HOME Button
#define IR_CODE_SIT "597b1824" //Info_Display Button
/*"4ab0f7b6")
 * 
 ...
 case 0xFF629D: Serial.println(" FORWARD"); break;
  ...
  /*
   * extra buttons on Xinda remote control
   * ok button ff02fd
   * *button ff42bd
   * # button ff52ad
   * 8 button ff38c7
   ...
   * 0 button ff4ab5
   */
   // Define a few commands from your remote control Xinda Remote Codes
#define IR_CODE_FORWARD "ff629d" //Up
#define IR_CODE_BACKWARDS "ffa857" //Down
#define IR_CODE_TURN_LEFT "ff22dd" //Left
#define IR_CODE_TURN_RIGHT "ffc23d" //Right
#define IR_CODE_FLAT_OUT "ff6897" //1
#define IR_CODE_STAND_UP "ff9867" //2
#define IR_CODE_LAY_DOWN "ffb04f" //3
#define IR_CODE_WINK "ff30cf" //4
#define IR_CODE_TAP_DANCE "ff18e7" //5
#define IR_CODE_STAND "ff7a85" //6
#define IR_CODE_SIT "ff10ef" //7
#define IR_CODE_SONAR_AVOID "ff42bd" //*
//#define IR_CODE_STRETCH "ff52ad" //#
#define IR_CODE_GYM "ff52ad"//#

boolean isActing=false;  //If the robot is executing command from remote
long timer;  
const long TIME_OUT=350;
...
void laydown() // lay down
{
  ...
  for(up = 0; up < 70; up++)
  {
    frl.write(home_frl-70+up);
    up2 = up2 - 1;
    fll.write(home_fll+70+up2);
    delay(20);
  }
  for(up = 0; up < 70; up++)
  {
    rll.write(home_rll-70+up);
    up3 = up3 - 1;
    rrl.write(home_rrl+70+up3);
    delay(15);
  }
}
/////////////////////////////////////
...
//  int processResult(String mycommand) { 
...
test();
  ...
  if (irrecv.decode(&results)) {
    ...
    Serial.print("The command before we past it on : ");
    Serial.println(command);
    if ( command == "ffffffff") {
     // sit();/////////////////////////////////////////almost
     loop();
      command = SaveForKeepPressingCommand;
      
    }
    else {
      SaveForKeepPressingCommand=command;
    }  
...
void loop()
{
  ...
  if (irrecv.decode(&results)) {
    ...
    Serial.println(command);
    if ( command == "ffffffff") {
      Serial.println("Repeat previous command");
      command = SaveForKeepPressingCommand;
    }
    else {
      SaveForKeepPressingCommand=command;
  ...
  if(isActing && (millis()-timer>=TIME_OUT)){
    Serial.println("Make robot stand still");
    stand();
    isActing=false;
  }
...
int processResult(String mycommand) { 
  ...
  if ( mycommand == IR_CODE_FORWARD) {
    Serial.println("Move forward");
    myuberkewl_forward();
    myuberkewl_forward();
    Serial.println("We finished moving forward");
    isActing=true;
    timer=millis();  //Continue the last action, reset timer
  }
  else if ( mycommand == IR_CODE_BACKWARDS) {
    Serial.println("Move backwards");
    myuberkewl_backward();
    isActing=true;
    timer=millis();  //Continue the last action, reset timer
  }
  else if ( mycommand == IR_CODE_TURN_LEFT) {
    Serial.println("Turn left");
    myuberkewl_leftturn();
    isActing=true;
    timer=millis();  //Continue the last action, reset timer
  }
  else if ( mycommand == IR_CODE_TURN_RIGHT) {
    Serial.println("Turn right");
    myuberkewl_rightturn();
    isActing=true;
    timer=millis();  //Continue the last action, reset timer
  }
  else if ( mycommand == IR_CODE_FLAT_OUT) {
    Serial.println("Flatout");
    flatout();
    isActing=false;
    timer=millis();  //Continue the last action, reset timer
  }
  else if ( mycommand == IR_CODE_STAND_UP) {
    Serial.println("Standup");
    standup();
    isActing=true;
    timer=millis();  //Continue the last action, reset timer
  }
  else if ( mycommand == IR_CODE_STAND) {
    Serial.println("Stand");
    stand();
    isActing=false;
    timer=millis();  //Continue the last action, reset timer
  }
  else if ( mycommand == IR_CODE_LAY_DOWN) {
    Serial.println("Laydown");
    laydown();
    isActing=false;
    timer=millis();  //Continue the last action, reset timer
  }
  else if ( mycommand == IR_CODE_WINK) {
    Serial.println("Wink");
    wink();
    isActing=true;
    timer=millis();  //Continue the last action, reset timer
  }
  else if ( mycommand == IR_CODE_TAP_DANCE) {
    Serial.println("Tap dance");
    tapdance();
    isActing=false;
    timer=millis();  //Continue the last action, reset timer
  }
  else if ( mycommand == IR_CODE_SIT) {
    Serial.println("Sit");
    sit();
    isActing=false;
    timer=millis();  //Continue the last action, reset timer
  ...
  else if ( mycommand == IR_CODE_SONAR_AVOID) {
    Serial.println("Sonar avoid");
    sonaravoid();
    isActing=false;
    timer=millis();  //Continue the last action, reset timer
  }
  else if ( mycommand == IR_CODE_GYM) {
    Serial.println("gym");
    gym();
    isActing=false;
    timer=millis();  //Continue the last action, reset timer
   ...
   */
    ...
    Serial.println("Continue last action till timer reset");
    timer=millis();  //Continue the last action, reset timer
  } 
